...
function consultarEpisodios(){	
	...
	if (documento != ""){		
		...
		var url = "http://172.24.99.138:8080/medical/services/servidor/consultar/" + documento;
		if (fechaInicio != "" && fechaFin != "" && $("#filtrar:checked").length > 0){
			fechaInicio = (new Date(fechaInicio)).toISOString();
			fechaFin = (new Date(fechaFin)).toISOString();
			url = "http://172.24.99.138:8080/medical/services/servidor/consultarF/" + documento + "/" + fechaInicio + "/" + fechaFin;
		}
		$.ajax({
			type: "GET",
			url: url,
...
function detalleEpisodio(obj){	
	...
	var url = "http://172.24.99.138:8080/medical/services/servidor/analisis/" + JSON.stringify(dataEpisodio);
	$.ajax({
		type: "GET",
		url: url,		
		dataType: "json",
